...
/src/db          schema.ts (Drizzle) · client.ts (open + initDb) · repo.ts (typed queries)
...
- **Verdict Day / checkpoint** — the re-evaluation moment shown when a sustained *declining* trend is detected. Three doors: ignore / adjust priorities / get support.
...
Not yet built / next: the ~30-day priorities check-in (hybrid cadence secondary path); crisis-tier vs BetterHelp signposting split; **eyes-on simulator pass of the brand round**; dev build to verify notifications + the Verdict "adjust → Settings" modal flow; Drizzle migrations (replace `initDb` bootstrap); Maestro e2e flows; validate the open `[DESIGN DECISION]`s in `docs/SPEC.md`.
