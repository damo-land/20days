...
/src/verdict     Verdict-Day narrator seam (tested): index.ts (generateNarrative: on-device model→templated fallback) · types.ts (contract) · questions.ts (per-category templated reflection Qs) · notes.ts (relevance-selects the user's own notes) · templatedNarrator.ts (deterministic floor) · appleNarrator.ts (Phase 2, INERT until activated — on-device Apple Foundation Models, no cloud)
